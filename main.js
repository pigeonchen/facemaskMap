...
// EXTERNAL MODULE: ./data/city.json
...
// EXTERNAL MODULE: ../node_modules/leaflet.locatecontrol/src/L.Control.Locate.js
var L_Control_Locate = __webpack_require__(29);

// EXTERNAL MODULE: ../node_modules/leaflet.markercluster/dist/leaflet.markercluster-src.js
var leaflet_markercluster_src = __webpack_require__(30);
...
document.querySelector('.tip').addEventListener('click', function () {
  var cardBox = document.querySelector('.cardBox').classList;
  cardBox.add('active');
});
document.querySelector('.cardBox').addEventListener('click', function (e) {
  e.preventDefault();
  e.currentTarget.classList.remove('active');
}); //-------------------顯示時間

var date = new Date(); //month 會從0開始算

var year = date.getFullYear();
var month = date.getMonth() + 1;
var day = date.getDate();
document.querySelector('.date').textContent = "".concat(year, " / ").concat(month, " / ").concat(day, " "); //--------------------option選項

var selectCity = document.querySelector('#selectCity');
var selectTown = document.querySelector('#selectTown');
...
(function ajaxData() {
  axios_default.a.get('https://raw.githubusercontent.com/kiang/pharmacies/master/json/points.json').then(function (res) {
    var response = res.data.features;
    var filterResponse = response.filter(function (element) {
      return element.properties.county === selectCity.value && element.properties.town === selectTown.value;
    });
    storeData = response;
...
function updateCounty(cityData) {
  ...
  for (var i = 0; i < cityData.length; i++) {
    str += "\n    <option style=\"border-radius: 20px\"\n    value=\"".concat(cityData[i].CityName, "\">").concat(cityData[i].CityName, "</option>\n\t\t");
  }
...
function updateTown(cityData) {
  var str = '';
  var townData = cityData.filter(function (city) {
    return city.CityName === selectCity.value;
  });

  for (var i = 0; i < townData[0].AreaList.length; i++) {
    str += "\n    <option value=\"".concat(townData[0].AreaList[i].AreaName, "\">\n    ").concat(townData[0].AreaList[i].AreaName, "</option>\n    ");
  }
...
function setMapPoint() {
  osmMap = leaflet_src_default.a.map('map', {
  ...
  leaflet_src_default.a.control.locate({
    showPopup: false
  }).addTo(osmMap).start();
}

function searchPharmacies(data) {
  var searchText = searchName.value;

  if (searchText === '') {
    return alert('請輸入資料');
  }

  var pharmacies = data.filter(function (pharmacy) {
    return pharmacy.properties.address.includes(searchText) || pharmacy.properties.name.match(searchText);
  });
  updateSidebar(pharmacies);
}

function selectPharmaciesArea(data) {
  var pharmacies = data.filter(function (element) {
    return element.properties.county === selectCity.value && element.properties.town === selectTown.value;
  });
  updateSidebar(pharmacies);
...
function updateMarker(data) {
  var markers = new leaflet_src_default.a.MarkerClusterGroup();
  data.forEach(function (pharmacy) {
    var geometry = pharmacy.geometry,
        properties = pharmacy.properties;

    var iconColor = function () {
      if (properties.mask_adult === 0) {
        return new leaflet_src_default.a.Icon({
    ...
    markers.addLayer(leaflet_src_default.a.marker([geometry.coordinates[1], geometry.coordinates[0]], {
      icon: iconColor
    }).bindPopup("\n      <li class=\"list-group-item  border-0\">\n        <h5 class=\"card-title\">".concat(properties.name, "</h5>\n        <p class=\" \">\u96FB\u8A71: ").concat(properties.phone, " </p>\n        <p class=\" \">\u5730\u5740: <a  href=\"https://www.google.com.tw/maps/place/").concat(properties.address, "\" target=\"_blank\">").concat(properties.address, "</a></p>\n        <p class=\" \">\u66F4\u65B0\u6642\u9593: ").concat(properties.updated, "</p>\n        <p class=\" mb-1 \">\u6CE8\u610F\u4E8B\u9805: ").concat(properties.custom_note === '' ? '無' : properties.custom_note, "</p>\n        <div class=\"mask d-flex justify-content-center mt-5\">\n          <a href=\"#\" class=\"btn btn-blue  mr-2 d-inline-block text-light\">\u6210\u4EBA\u53E3\u7F69:").concat(properties.mask_adult, "</a>\n          <a href=\"#\" class=\"btn btn-pink  d-inline-block text-light\">\u5152\u7AE5\u53E3\u7F69: ").concat(properties.mask_child, "</a>\n        </div>\n      </li>")));
...
function updateSidebar(data) {
  var str = '';
  data.forEach(function (pharmacy) {
    var properties = pharmacy.properties;
    str += "\n    <li class=\"list-group-item \">\n      <h5 class=\"card-title\">".concat(properties.name, "</h5>\n      <p class=\" mb-1\">\u96FB\u8A71: ").concat(properties.phone, " </p>\n      <p class=\" mb-1\">\u5730\u5740: <a  href=\"https://www.google.com.tw/maps/place/").concat(properties.address, "\" target=\"_blank\">").concat(properties.address, "</a></p>\n      <p class=\" mb-1 \">\u66F4\u65B0\u6642\u9593: ").concat(properties.updated, "</p>\n      <p class=\" mb-1 \">\u6CE8\u610F\u4E8B\u9805: ").concat(properties.custom_note === '' ? '無' : properties.custom_note, "</p>\n      <div class=\"mask d-flex justify-content-between mt-5\">\n        <a href=\"#\" class=\"btn btn-blue w-50 mr-2 text-light\">\u6210\u4EBA\u53E3\u7F69:").concat(properties.mask_adult, "</a>\n        <a href=\"#\" class=\"btn btn-pink w-50 text-light\">\u5152\u7AE5\u53E3\u7F69: ").concat(properties.mask_child, "</a>\n      </div>\n    </li>");
  });
  listGroup.innerHTML = str; // 增加List click事件

  document.querySelectorAll('.list-group-item').forEach(function (element, index) {
...
function panToMarker(data) {
  ...
  leaflet_src_default.a.marker([lat, lon]).addTo(osmMap).bindPopup("\n      <li class=\"list-group-item  border-0\">\n        <h5 class=\"card-title\">".concat(properties.name, "</h5>\n        <p class=\" \">\u96FB\u8A71: ").concat(properties.phone, " </p>\n        <p class=\" \">\u5730\u5740: <a  href=\"https://www.google.com.tw/maps/place/").concat(properties.address, "\" target=\"_blank\">").concat(properties.address, "</a></p>\n        <p class=\" \">\u66F4\u65B0\u6642\u9593: ").concat(properties.updated, "</p>\n        <p class=\" mb-1 \">\u6CE8\u610F\u4E8B\u9805: ").concat(properties.custom_note === '' ? '無' : properties.custom_note, "</p>\n        <div class=\"mask d-flex justify-content-between mt-5\">\n          <a href=\"#\" class=\"btn btn-blue w-50 mr-1 d-inline-block text-light\">\u6210\u4EBA\u53E3\u7F69:").concat(properties.mask_adult, "</a>\n          <a href=\"#\" class=\"btn btn-pink w-50 d-inline-block text-light\">\u5152\u7AE5\u53E3\u7F69: ").concat(properties.mask_child, "</a>\n        </div>\n      </li>")).openPopup();
}
...
// CONCATENATED MODULE: ./img/tips.png
/* harmony default export */ var tips = (__webpack_require__.p + "img/tips.png");
